...
class Header extends Component {
    ...
    getApi() {
        const getWebsite = fetch('http://localhost:8081/api/getWebsite')
            .then(response => response.json())
            .then(result => result);

        const getTopNav = fetch('http://localhost:8081/api/getTopNav')
            .then(response => response.json())
            .then(result => result);

        Promise.all([getWebsite, getTopNav]).then(
            result => {
                // console.log(result);
                let [website, topNav] = result;
                // console.log(topNav);
                this.setState({ website, topNav });
            },
            error => {
                console.log(error);
            }
    ...
    componentDidMount() {
        // 展开导航栏
        this.refs.navOpen.ontouchstart = event => {
            event.stopPropagation();
            this.refs.navOpen.style.animation = 'hidden 0.3s linear forwards';
            this.refs.navClose.style.animation =
                'show 0.3s 0.3s linear forwards';
            this.refs.navList.style.animation =
                'slideDown 0.3s linear forwards';
        };
        // 收起导航栏
        this.refs.navClose.ontouchstart = event => {
            this.refs.navOpen.style.animation =
                'show 0.3s 0.4s linear forwards';
    ...
    render() {
        let website = this.state.website ? this.state.website : [];
        let topNav = this.state.topNav ? this.state.topNav : [];

        return (
            <div className="header">
                <Link className="logo">
                    <img src={website.logo_mobile} alt="" />
                    {/* <svg
                        height="30"
